...
public class OrdersConfigurationProperties {
    ...
    public URI getPaymentUri() {
        return new ServiceUri(new Hostname("dispatcher.default.svc.cluster.local:80/function/openfaas-fn/payment"), new Domain(domain), "/paymentAuth").toUri();
    }

    public URI getShippingUri() {
        return new ServiceUri(new Hostname("dispatcher.default.svc.cluster.local:80/function/openfaas-fn/shipping"), new Domain(domain), "/shipping").toUri();
    }
